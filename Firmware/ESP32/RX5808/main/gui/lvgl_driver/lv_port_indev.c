...
//     gpio_pullup_en(0);
    ...
    // adc1_config_channel_atten(KEY_ADC_CHAN, ADC_EXAMPLE_ATTEN);
    /*Your code comes here*/
}
...
static uint32_t keypad_get_key(void)
{
    ...
    if(key_raw<2200&&key_raw>1750)
        return LV_KEY_ENTER;
#else
    if(key_raw<3230&&key_raw>2389)	
        return LV_KEY_UP;
    if(key_raw<1649&&key_raw>995)
        return LV_KEY_DOWN;
    if(key_raw<300)	
        return LV_KEY_LEFT;
    if(key_raw<2389&&key_raw>1649)	
        return LV_KEY_RIGHT;
   if(key_raw<995&&key_raw>300)
        return LV_KEY_ENTER;
#endif
    return 0;
}
